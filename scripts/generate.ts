...
import type { GeneratedBundle } from '../src/parsers/types.ts'
...
async function generateForDeployment(config: DeploymentConfig): Promise<void> {
  ...
  // The cast is intentional: JSON literals have widened string types that don't satisfy
  // the union types in GeneratedBundle. Correctness is enforced by the `output: GeneratedBundle`
  // type annotation above, which catches shape mismatches at bake time.
  const fileContent = [
    ...
    `export const bundleData = ${JSON.stringify(output)} as unknown as GeneratedBundle`,
    '',
  ].join('\n')
